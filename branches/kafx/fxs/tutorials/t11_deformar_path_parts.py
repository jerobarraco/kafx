...
class EfectoGenerico(common.Fx):
	def __init__(self):
		...
		self.parts = advanced.cParticleSystem(png="textures/spark5.png",
	...
	def deform(self, sil, vector):
		...
		for t, p in vector:
			...
			if (l == 2) or (l == 6):
				if l ==2:
					px = common.LERP(sil.progress, last[0], p[0])
					py = common.LERP(sil.progress, last[1], p[1])
					last = p
				else:
					px, py = common.PointBezier(
